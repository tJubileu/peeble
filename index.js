...
client.on("guildCreate", server => {
    const sEmbed = new RichEmbed() 
        ...
        .addField("Dono(a)", server.owner.user.username, true)
        .addField("ID do Dono", server.owner.user.id, true)
        .addField("Chats", server.channels.size, true)
        .setFooter("Criado em " + moment(server.createdAt).format("LLL"))
        .setTimestamp()
    client.channels.find(c => c.id === "654359345823219752").send(sEmbed)
})
client.on("messageUpdate", async (oldMessage, newMessage) => {
    try {
        ...
        if (db.get(`${message.author.id}.active`) == null) db.set(`${message.author.id}.active`, true)
        if (db.get(`${message.author.id}.active`) != null && db.get(`${message.author.id}.active`) != true) db.set(`${message.author.id}.active`, true)

        if (prefix == null) db.set(`${message.guild.id}.prefix`, '+') 
        if (message.channel.type == "dm" && message.author.id !== "614498625023770659") {
            const embaq = new RichEmbed()
                .setTitle("PV")
                .setAuthor(message.author.tag + ` (${message.author.id})`, message.author.avatarURL)
                .addField("Mensagem", `\`${message.content}\``, true)  
                .setTimestamp()
            return client.channels.get('657015213375356950').send(embaq)
        }
        ...
        if (message.isMentioned(client.user) && !args[0] && message.content.startsWith("<61449862502")) return message.channel.send(`${message.author} ➤ Olá! Me chamo **Peeble**, e meu prefix neste servidor é \`${prefix}\`! Para saber mais sobre mim, você pode usar o comando \`${prefix}ajuda\`!`)
        if (message.content.indexOf(prefix) !== 0) return;

        let cooldownz = (600000)
        const user = message.author
        let lastDailyz = await db.fetch(`PrisonTime_${user.id}`)
        if (lastDailyz !== null && cooldownz - (Date.now() - lastDailyz) > 0) {
            let timeObj = ms(cooldownz - (Date.now() - lastDailyz) )
         ...
         //#region NECESSARY TO COMMAND HANDLER WORKs                               d                                                             
